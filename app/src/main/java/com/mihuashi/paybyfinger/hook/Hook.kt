...
object Hook : BaseHook() {
       ...
       loadClassOrNull("com.stub.StubApp").isNotNull {
            it.methodFinder().first { name == "attachBaseContext" }.createHook {
                after { param ->
                    ...
                    sharedPreferences = context.getSharedPreferences("mhshooksetting", Context.MODE_PRIVATE) // 用来保存设置
                    ...
                    val resultReceiver = object : BroadcastReceiver() {
                        override fun onReceive(context: Context?, intent: Intent?) {
                            ...
                            if (result) {
                                ...
                                val pass = context?.let { it1 -> decryptData(alias, it1) }
                                if (BuildConfig.DEBUG) {
    ...
    fun executeCustomFunction(fragmentInstance: Any, classLoader: ClassLoader) {
        try {
            ...
            if (rootView != null) {
                ...
                val itemView = rootView.findViewById<ViewGroup>(mineSettingEmployerRss) as FrameLayout
    ...
    fun setui (classLoader: ClassLoader, subViewLinearLayout: LinearLayout, context: Context,bctivity:Activity){
            ...
            val viewa = findClass("com.qixin.mihuas.module.main.mine.widget.MineSettingItemView", classLoader)
            val switchclass = findClass("com.qixin.mihuas.resource.widget.text.switchbutton.SwitchButtonView", classLoader)
            ...
            name.layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply {
            ...
            roundedLinearLayout.layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply {
